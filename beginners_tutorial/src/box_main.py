...
def listener():
 ...
 #   pub_shape=rospy.Publisher('box_shape',Bool,queue_size=1000)
    puby=rospy.Publisher('y_distace',Float32,queue_size=1000)
   # pubangle=rospy.Publisher('line_angle',Float32,queue_size=1000)
    ...
    while (cap.isOpened()):
            ...
            ret,thresh1=cv2.threshold(hsv,155,255,cv2.THRESH_BINARY_INV)
            low_a=np.array([100,100,70])
            high_a=np.array([120,120,100])
            mask=cv2.inRange(thresh1,low_a,high_a)
            mask_not=cv2.bitwise_not(mask)
            res=cv2.bitwise_and(thresh1,thresh1,mask=mask_not)
